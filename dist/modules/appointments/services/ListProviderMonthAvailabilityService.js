...
let ListProviderMonthAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const numberOfDaysInMonth = (0, _dateFns.getDaysInMonth)(new Date(year, month - 1)); // For each day of the month

    const eachDayArray = Array.from({
      length: numberOfDaysInMonth
    }, (_, index) => index + 1); // console.log(eachDayArray);
    // Checks for appointments on certain days

    const availability = eachDayArray.map(day => {
      const compareDate = new Date(year, month - 1, day, 23, 59, 59); // Appointments of the day specific
