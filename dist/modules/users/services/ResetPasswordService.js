...
let ResetPasswordService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const user = await this.usersRepository.findById(userToken === null || userToken === void 0 ? void 0 : userToken.user_id);

    if (!user) {
      throw new _AppError.default('User does not exists');
    }

    const tokenCreatedAt = userToken.created_at; // Check the creation time of the token until its validity
